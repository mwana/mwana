...
HELP_TEXT = _("To report first vaccination for a child send <SESSION5> <Baby ID> <DATE OF VACCINATION> e.g SESSION5  123/16 23/8/2016")


class SessionFiveHandler(KeywordHandler, Parser):    

    keyword = "SESSION 5|SESSION FIVE|S 5|S FIVE|SESSION5|SESSIONFIVE|S5|SFIVE"
    ...
    def handle(self, text):
        ...
        session5 = VaccinationSession.objects.get(session_id='s5')

        appointment = Appointment.objects.get(client=client, vaccination_session=session5)
        ...
        self.respond(_("Thank you %(cba)s! You have reported '%(s5)s' for baby with "
                       "ID %(id)s and vaccination date %(vac_date)s."),
                     cba=self.msg.contact.name,
                     id = client.client_number,
                     s5=session5.__unicode__(),
