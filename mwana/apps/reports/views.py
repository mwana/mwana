...
def contacts_report(request):
    ...
    (facility_contacts, messages_paginator_num_pages, messages_number, messages_has_next,
     messages_has_previous) = r.facility_contacts_report(page)
    ...
    return render_to_response('reports/contacts.html',
                              {
                                  ...
                                  'rpt_group': get_groups_dropdown_html('rpt_group', rpt_group),
                                  'rpt_provinces': get_facilities_dropdown_html("rpt_provinces",
                                                                                r.get_rpt_provinces(request.user),
                                                                                rpt_provinces),
                                  'rpt_districts': get_facilities_dropdown_html("rpt_districts",
                                                                                r.get_rpt_districts(request.user),
                                                                                rpt_districts),
                                  'rpt_facilities': get_facilities_dropdown_html("rpt_facilities",
                                                                                 r.get_rpt_facilities(request.user),
                                                                                 rpt_facilities),
                              }, context_instance=RequestContext(request))
...
def supported_sites(request):
    ...
    for record in sorted(records, key=lambda record: record.parent.parent.name.lower()):
        ...
        site.cbas = Contact.active.filter(types=get_cba_type(), is_active=True,
                                          location__parent=record).distinct().count()
        site.dhos = Contact.active.filter(types=get_district_worker_type(), is_active=True,
                                          location__location=record).distinct().count()
        site.phos = Contact.active.filter(types=get_province_worker_type(), is_active=True,
                                          location__location=record).distinct().count()
        site.printers = Contact.active.filter(types=get_dbs_printer_type(), is_active=True,
                                              location=record).distinct().count()
        ...
        site.sample_sent_to_lab_this_month = Result.objects.filter(clinic=record,
                                                                   entered_on__year=today.year,
                                                                   entered_on__month=today.month).count()

        site.results_retrieved_this_month = Result.objects.filter(clinic=record,
                                                                  result_sent_date__year=today.year,
                                                                  result_sent_date__month=today.month).count()
        ...
        site.births_this_month = PatientEvent.objects.filter(patient__location__parent=record,
                                                             date_logged__year=today.year,
                                                             date_logged__month=today.month).count()
...
def home(request):
    ...
    sites = []
    unsupported_districts = [str(loc.slug) for loc in Location.objects.filter(type__slug='districts').exclude(
        location__supportedlocation__supported=True)]
