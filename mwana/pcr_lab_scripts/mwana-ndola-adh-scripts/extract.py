...
def archive_old_facility_samples (lookback, facility_list):
  ...
  conn = dbconn('prod')
  curs = conn.cursor()
  sql = "select distinct LabID, Facility from tbl_Patient where DateReceived < ?"
  ...
  for rec in records:
    try:
        curs.execute("insert into samples (sample_id, facility_code, sync_status) values (?, ?, 'historical')", [norm_id(rec[0]), rec[1]])
        arcvd.append(rec[1])
...
def enable_network ():
  run_rasdial('sms /phone:*99***1#')
  time.sleep(10)

def disable_network ():
  run_rasdial('sms /disconnect')
